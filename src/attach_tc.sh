...
if [ "${DIRECTION}" ] ; then
    ...
    sudo tc filter add dev ${NIC} ${DIRECTION} bpf direct-action object-file $(which ${BINARY}_${DIRECTION}_tc.o) sec .text
else
    echo "### Loading ${BINARY}_ingress_tc.o to ${NIC} ingress ###"
    sudo tc filter add dev ${NIC} ingress bpf direct-action object-file $(which ${BINARY}_ingress_tc.o) sec .text
    echo "### Loading ${BINARY}_egress_tc.o to ${NIC} egress ###"
    sudo tc filter add dev ${NIC} egress  bpf direct-action object-file $(which ${BINARY}_egress_tc.o) sec .text
fi
