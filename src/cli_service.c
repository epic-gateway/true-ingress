...
void usage(char *prog) {
    ...
    fprintf(stderr,"    %s get <proto> <ip-proxy> <port-proxy> <ip-ep> <port-ep> <service-id> <group-id>\n", prog);
    fprintf(stderr,"    %s set <proto> <ip-proxy> <port-proxy> <ip-ep> <port-ep> <service-id> <group-id> <tunnel-id> <key>\n", prog);
    fprintf(stderr,"    %s del <proto> <ip-proxy> <port-proxy> <ip-ep> <port-ep> <service-id> <group-id>\n", prog);
}
...
bool map_verify_get(int map_fd, struct identity *key, struct key *value) {
    if (bpf_map_lookup_elem(map_fd, key, value)) {
...
bool map_verify_set(int map_fd, struct identity *key, struct key *value) {
    if (bpf_map_update_elem(map_fd, key, value, 0)) {
...
int main(int argc, char **argv)
{
    ...
    if (!strncmp(argv[1], "set", 4)) { // %s set <proto> <ip-proxy> <port-proxy> <ip-ep> <port-ep> <service-id> <group-id> <tunnel-id> <key>
        if (argc < 11) {
            usage(argv[0]);
            return 1;
        }
        ...
        // dnat
        map_nat_set(map_nat_fd, make_endpoint(&ep_from, from.s_addr, atoi(argv[4]), proto), make_endpoint(&ep_to, to.s_addr, atoi(argv[6]), proto));
        // snat
        ...
        map_encap_set(map_encap_fd, &ep_to, make_service(&svc, atoi(argv[10]), make_identity(&id, atoi(argv[7]), atoi(argv[8])), &pwd));
        ...
        if (!strncmp(argv[2], "all", 4)) {
            ...
            struct endpoint ep_from, ep_to, value;
            // dnat
            map_nat_get(map_nat_fd, make_endpoint(&ep_from, from.s_addr, atoi(argv[4]), proto), &value);
            // snat
            map_nat_get(map_nat_fd, make_endpoint(&ep_to, to.s_addr, atoi(argv[6]), proto), &value);

            struct service svc;
            struct identity id;
            struct key pwd;

            map_encap_get(map_encap_fd, &ep_to, &svc);
            map_verify_get(map_verify_fd, make_identity(&id, atoi(argv[7]), atoi(argv[8])), &pwd);
        }
        ...
        if (!strncmp(argv[2], "all", 4)) {
            ...
            struct endpoint ep_from, ep_to;
            // dnat
            map_nat_del(map_nat_fd, make_endpoint(&ep_from, from.s_addr, atoi(argv[4]), proto));
            // snat
            map_nat_del(map_nat_fd, make_endpoint(&ep_to, to.s_addr, atoi(argv[6]), proto));

            struct identity id;

            map_encap_del(map_encap_fd, &ep_to);
            map_verify_del(map_verify_fd, make_identity(&id, atoi(argv[7]), atoi(argv[8])));
        }
    } else {
