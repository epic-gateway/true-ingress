...
if [ "${VERBOSE}" ]; then
    echo -e "\nHTTP(${SERVICE_ID}).START : NODE='${NODE}' IP='${IP}' PORT='${PORT}' SERVICE_ID='${SERVICE_ID}' HOME='${HOME}'"
fi
...
echo -e "\n==============================================="
echo "# HTTP(${SERVICE_ID}).START [1/${STEPS}] : Check input"
...
echo -e "\n==============================================="
echo "# HTTP(${SERVICE_ID}).START [2/${STEPS}] : Setup service IP ${IP} on '${NODE}'"
docker exec -it ${NODE} bash -c "ip addr add ${IP} dev lo"
docker exec -it ${NODE} bash -c "ip addr"

echo -e "\n==============================================="
echo "# HTTP(${SERVICE_ID}).START [3/${STEPS}] : Starting HTTP server ID ${SERVICE_ID} on ${IP}:${PORT}) on '${NODE}'"
...
echo -e "\n==============================================="
echo "# HTTP(${SERVICE_ID}).START [4/${STEPS}] : Wait about ${DELAY} seconds for HTTP server going up"
sleep ${DELAY}

echo -e "\n==============================================="
echo "# HTTP(${SERVICE_ID}).START [5/${STEPS}] : Check service is running"
...
echo -e "\n==============================================="
echo "# HTTP(${SERVICE_ID}).START : DONE"
