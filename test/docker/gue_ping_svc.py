...
def usage():
        ...
        print("\nExample : %s eth0 60 5.5.5.5 6080 6080 5 '01010101010101010101010101010101'" % argv[0])
...
def main(argv):
    ...
    try:
        print("Starting infinite loop to send GUE Control packet for group-id %s to %s:%s every %d second(s)" % (argv[6], argv[3], argv[4], delay))
        while (1):
            print(".")

            #print("python3 /tmp/.acnodal/bin/gue_ping_svc_once.py %s %s %s %s 0 %s '%s'" % (argv[1], argv[3], argv[4], argv[5], argv[6], argv[7]))
            os.popen("python3 /tmp/.acnodal/bin/gue_ping_svc_once.py %s %s %s %s 0 %s '%s'" % (argv[1], argv[3], argv[4], argv[5], argv[6], argv[7]))
