...
if [ "${VERBOSE}" ]; then
    ...
    docker exec -it ${NODE} bash -c "/tmp/.acnodal/bin/pfc_start.sh ${NODE_NIC} "${NODE}" 9 8 ${NODE_PORT_MIN} ${NODE_PORT_MAX} ${DELAY} 10 100"
    ...
    echo "  ${NODE}"
    docker exec -it ${NODE} bash -c "/tmp/.acnodal/bin/pfc_start.sh ${NODE_NIC} "${NODE}" 9 8 ${NODE_PORT_MIN} ${NODE_PORT_MAX} ${DELAY} 10 100" > /dev/null
fi
...
if [ "${VERBOSE}" ]; then
    ...
    docker exec -it ${PROXY2} bash -c "/tmp/.acnodal/bin/pfc_add.sh ${PROXY_NIC2} ${GROUP_ID} ${SERVICE_ID2} ${PASSWD2} 0 0 ${SERVICE_PROTO} ${PROXY_IP2} ${PROXY_PORT2} ${SERVICE_IP} ${SERVICE_PORT}"
    ...
    docker exec -it ${NODE} bash -c "/tmp/.acnodal/bin/pfc_add.sh -p ${NODE_NIC} ${GROUP_ID} ${SERVICE_ID2} ${PASSWD2} ${PROXY_TUN_IP2} ${PROXY_TUN_PORT2} ${SERVICE_PROTO} ${PROXY_IP2} ${PROXY_PORT2} ${SERVICE_IP} ${SERVICE_PORT}"
    ...
    docker exec -it ${PROXY2} bash -c "/tmp/.acnodal/bin/pfc_add.sh ${PROXY_NIC2} ${GROUP_ID} ${SERVICE_ID2} ${PASSWD2} 0 0 ${SERVICE_PROTO} ${PROXY_IP2} ${PROXY_PORT2} ${SERVICE_IP} ${SERVICE_PORT}" > /dev/null
    PROXY_TUN_PORT2=$(docker exec -it ${PROXY2} bash -c "/tmp/.acnodal/bin/cli_tunnel get ${TUNNEL_ID2}" | grep ${TUNNEL_ID2} | awk '{print $3}' | sed 's/:/ /g' | awk '{print $2}')
    docker exec -it ${NODE} bash -c "/tmp/.acnodal/bin/pfc_add.sh -p ${NODE_NIC} ${GROUP_ID} ${SERVICE_ID2} ${PASSWD2} ${PROXY_TUN_IP2} ${PROXY_TUN_PORT2} ${SERVICE_PROTO} ${PROXY_IP2} ${PROXY_PORT2} ${SERVICE_IP} ${SERVICE_PORT}" > /dev/null
fi
