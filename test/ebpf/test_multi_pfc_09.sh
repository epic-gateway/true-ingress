...
if [ "${VERBOSE}" ]; then
    # service_add.sh  <nic> <group-id> <service-id> <passwd> <remote-tunnel-ip> <remote-tunnel-port> <proto> <proxy-ip> <proxy-port> <backend-ip> <backend-port>
    ...
    docker exec -it ${NODE2} bash -c "/tmp/.acnodal/bin/pfc_add.sh -p ${NODE_NIC2} ${GROUP_ID} ${SERVICE_ID2} ${PASSWD2} ${PROXY_TUN_IP} ${PROXY_TUN_PORT2} ${SERVICE_PROTO2} ${PROXY_IP} ${PROXY_PORT2} ${SERVICE_IP2} ${SERVICE_PORT2}"
    ...
    PROXY_TUN_PORT2=$(docker exec -it ${PROXY} bash -c "/tmp/.acnodal/bin/cli_tunnel get ${TUNNEL_ID2}" | grep ${TUNNEL_ID2} | awk '{print $3}' | sed 's/:/ /g' | awk '{print $2}')
    docker exec -it ${NODE2} bash -c "/tmp/.acnodal/bin/pfc_add.sh -p ${NODE_NIC2} ${GROUP_ID} ${SERVICE_ID2} ${PASSWD2} ${PROXY_TUN_IP} ${PROXY_TUN_PORT2} ${SERVICE_PROTO2} ${PROXY_IP} ${PROXY_PORT2} ${SERVICE_IP2} ${SERVICE_PORT2}" > /dev/null
fi
