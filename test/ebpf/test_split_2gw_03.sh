...
if [ "${VERBOSE}" ]; then
    ...
    docker exec -it ${NODE} bash -c "pfc_add.sh ${NODE_NIC} ${GROUP_ID} ${SERVICE_ID2} ${PASSWD2} ${PROXY_TUN_IP2} ${PROXY_TUN_PORT2} ${SERVICE_PROTO} ${PROXY_IP2} ${PROXY_PORT2} ${SERVICE_IP} ${SERVICE_PORT}"
    ...
    PROXY_TUN_PORT2=$(docker exec -it ${PROXY2} bash -c "cli_tunnel get ${TUNNEL_ID2}" | grep ${TUNNEL_ID2} | awk '{print $3}' | sed 's/:/ /g' | awk '{print $2}')
    docker exec -it ${NODE} bash -c "pfc_add.sh ${NODE_NIC} ${GROUP_ID} ${SERVICE_ID2} ${PASSWD2} ${PROXY_TUN_IP2} ${PROXY_TUN_PORT2} ${SERVICE_PROTO} ${PROXY_IP2} ${PROXY_PORT2} ${SERVICE_IP} ${SERVICE_PORT}" > /dev/null
fi
