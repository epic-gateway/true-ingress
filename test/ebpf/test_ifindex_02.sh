...
#./proxy_add.sh ${PROXY} "2" 10.0.0.2 br0 2
#./proxy_add.sh ${PROXY} "3" 10.0.0.3 br0 3
#./proxy_add.sh ${PROXY} "4" 10.0.0.4 br0 4
#./proxy_add.sh ${PROXY} "5" 10.0.0.5 br0 5
#./proxy_add.sh ${PROXY} "6" 10.0.0.6 br0 6
#./proxy_add.sh ${PROXY} "7" 10.0.0.7 br0 7
#./proxy_add.sh ${PROXY} "8" 10.0.0.8 br0 8
#./proxy_add.sh ${PROXY} "9" 10.0.0.9 br0 9
#./proxy_add.sh ${PROXY} "10" 10.0.0.10 br0 10
# INFRA >>>> Setup PROXY NS1 -> create namespace, attach veth1, add it to bridge, assign public ip 5.5.5.5
./proxy_add.sh ${PROXY} "${PROXY_IP}" ${PROXY_IP} br0 1
# <<<<
...
docker exec -it ${PROXY} bash -c "ip netns exec proxy${PROXY_IP} ip route"
...
if [ "${VERBOSE}" ]; then
    echo ""
    docker exec -it ${PROXY} bash -c "ip netns exec proxy${PROXY_IP} iptables -t nat -L PREROUTING -vn --line-numbers"
fi
# >>>>
...
#docker exec -it ${PROXY} bash -c "ip netns exec proxy${PROXY_IP} iptables -t nat -L PREROUTING -vn --line-numbers"
#docker exec -it ${PROXY} bash -c "ip netns exec proxy${PROXY_IP} iptables -t nat -L POSTROUTING -vn --line-numbers"
# <<<<
