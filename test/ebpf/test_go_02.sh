...
if [ "${VERBOSE}" ]; then
    ...
    docker exec -it ${PROXY} bash -c "pfc_cli_go add ${PROXY_NIC} ${GROUP_ID} ${SERVICE_ID} ${PASSWD} ${SERVICE_PROTO} ${PROXY_IP} ${PROXY_PORT} ${SERVICE_IP} ${SERVICE_PORT} 0.0.0.0 0"
...
if [ "${VERBOSE}" ]; then
    docker exec -it ${PROXY} bash -c "pfc_cli_go del ${GROUP_ID} ${SERVICE_ID}"
    docker exec -it ${NODE} bash -c "pfc_cli_go del ${GROUP_ID} ${SERVICE_ID}"
    docker exec -it ${PROXY} bash -c "pfc_cli_go del ${GROUP_ID} ${SERVICE_ID2}"
    docker exec -it ${NODE} bash -c "pfc_cli_go del ${GROUP_ID} ${SERVICE_ID2}"
    ...
    docker exec -it ${PROXY} bash -c "pfc_cli_go del ${GROUP_ID} ${SERVICE_ID}" > /dev/null
    docker exec -it ${NODE} bash -c "pfc_cli_go del ${GROUP_ID} ${SERVICE_ID}" > /dev/null
    docker exec -it ${PROXY} bash -c "pfc_cli_go del ${GROUP_ID} ${SERVICE_ID2}" > /dev/null
    docker exec -it ${NODE} bash -c "pfc_cli_go del ${GROUP_ID} ${SERVICE_ID2}" > /dev/null
