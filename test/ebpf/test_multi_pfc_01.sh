...
if [ "${VERBOSE}" ]; then
    ...
    docker exec -it ${NODE} bash -c "pfc_add.sh -p ${NODE_NIC} ${GROUP_ID} ${SERVICE_ID2} ${PASSWD2} ${PROXY_TUN_IP} ${PROXY_TUN_PORT2} ${SERVICE_PROTO2} ${PROXY_IP} ${PROXY_PORT2} ${SERVICE_IP2} ${SERVICE_PORT2}"
    ...
    PROXY_TUN_PORT2=$(docker exec -it ${PROXY} bash -c "cli_tunnel get ${TUNNEL_ID2}" | grep ${TUNNEL_ID2} | awk '{print $3}' | sed 's/:/ /g' | awk '{print $2}')
    docker exec -it ${NODE} bash -c "pfc_add.sh -p ${NODE_NIC} ${GROUP_ID} ${SERVICE_ID2} ${PASSWD2} ${PROXY_TUN_IP} ${PROXY_TUN_PORT2} ${SERVICE_PROTO2} ${PROXY_IP} ${PROXY_PORT2} ${SERVICE_IP2} ${SERVICE_PORT2}" > /dev/null
fi
