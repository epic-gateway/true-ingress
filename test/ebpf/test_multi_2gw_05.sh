...
if [ "${VERBOSE}" ]; then
    ...
    docker exec -it ${PROXY2} bash -c "/tmp/.acnodal/bin/pfc_start.sh ${PROXY_NIC2} "${PROXY2}" 9 9 ${PROXY_PORT_MIN2} ${PROXY_PORT_MAX2}"
    docker exec -it ${NODE} bash -c "/tmp/.acnodal/bin/pfc_start.sh ${NODE_NIC} "${NODE}" 9 8 ${NODE_PORT_MIN} ${NODE_PORT_MAX} ${DELAY}"
    docker exec -it ${NODE2} bash -c "/tmp/.acnodal/bin/pfc_start.sh ${NODE_NIC2} "${NODE2}" 9 8 ${NODE_PORT_MIN2} ${NODE_PORT_MAX2} ${DELAY2}"

    docker exec -it ${NODE} bash -c "ps aux | grep 'gue_ping'"
    docker exec -it ${NODE2} bash -c "ps aux | grep 'gue_ping'"
else
    echo "Starting PFC..."
    echo "  ${PROXY}"
    docker exec -it ${PROXY} bash -c "/tmp/.acnodal/bin/pfc_start.sh ${PROXY_NIC} "${PROXY}" 9 9 ${PROXY_PORT_MIN} ${PROXY_PORT_MAX}" > /dev/null
    echo "  ${PROXY2}"
    docker exec -it ${PROXY2} bash -c "/tmp/.acnodal/bin/pfc_start.sh ${PROXY_NIC2} "${PROXY2}" 9 9 ${PROXY_PORT_MIN2} ${PROXY_PORT_MAX2}" > /dev/null
    echo "  ${NODE}"
    docker exec -it ${NODE} bash -c "/tmp/.acnodal/bin/pfc_start.sh ${NODE_NIC} "${NODE}" 9 8 ${NODE_PORT_MIN} ${NODE_PORT_MAX} ${DELAY}" > /dev/null
    echo "  ${NODE2}"
    docker exec -it ${NODE2} bash -c "/tmp/.acnodal/bin/pfc_start.sh ${NODE_NIC2} "${NODE2}" 9 8 ${NODE_PORT_MIN2} ${NODE_PORT_MAX2} ${DELAY}" > /dev/null
fi
...
echo "  ${SERVICE_NAME2}"
echo "    Proxy   : ${PROXY2}  ${SERVICE_PROTO2}:${PROXY_IP2}:${PROXY_PORT2} -> ${NODE2}  ${SERVICE_PROTO2}:${SERVICE_IP2}:${SERVICE_PORT2}"
echo "    Service : (${GROUP_ID},${SERVICE_ID2}) -> '${PASSWD2}'"
echo "    Tunnel  : ${TUNNEL_ID2} ${PROXY_TUN_IP2}:${PROXY_TUN_PORT2} -> ${NODE_TUN_IP2}:${NODE_TUN_PORT2}"
...
docker exec -it ${PROXY2} bash -c "iptables -t nat -A PREROUTING -p ${SERVICE_PROTO2} -i ${PROXY_NIC2} --destination ${PROXY_IP2} --dport ${PROXY_PORT2} -j DNAT --to-destination ${SERVICE_IP2}:${SERVICE_PORT2}"
docker exec -it ${PROXY2} bash -c "iptables -t nat -A POSTROUTING -p ${SERVICE_PROTO2} -o ${PROXY_NIC2} -s ${SERVICE_IP2} --sport ${SERVICE_PORT2} -j SNAT --to-source ${PROXY_IP2}:${PROXY_PORT2}"
# check
if [ "${VERBOSE}" ]; then
    echo ""
...
if [ "${VERBOSE}" ]; then
    ...
    docker exec -it ${PROXY2} bash -c "/tmp/.acnodal/bin/pfc_add.sh ${PROXY_NIC2} ${GROUP_ID} ${SERVICE_ID2} ${PASSWD2} 0 0 ${SERVICE_PROTO2} ${PROXY_IP2} ${PROXY_PORT2} ${SERVICE_IP2} ${SERVICE_PORT2}"
    PROXY_TUN_PORT2=$(docker exec -it ${PROXY2} bash -c "/tmp/.acnodal/bin/cli_tunnel get ${TUNNEL_ID2}" | grep ${TUNNEL_ID2} | awk '{print $3}' | sed 's/:/ /g' | awk '{print $2}')
    docker exec -it ${NODE2} bash -c "/tmp/.acnodal/bin/pfc_add.sh -p ${NODE_NIC2} ${GROUP_ID} ${SERVICE_ID2} ${PASSWD2} ${PROXY_TUN_IP2} ${PROXY_TUN_PORT2} ${SERVICE_PROTO2} ${PROXY_IP2} ${PROXY_PORT2} ${SERVICE_IP2} ${SERVICE_PORT2}"

    docker exec -it ${PROXY} bash -c "/tmp/.acnodal/bin/pfc_list.sh"
    docker exec -it ${PROXY2} bash -c "/tmp/.acnodal/bin/pfc_list.sh"
    docker exec -it ${NODE} bash -c "/tmp/.acnodal/bin/pfc_list.sh"
    docker exec -it ${NODE2} bash -c "/tmp/.acnodal/bin/pfc_list.sh"
else
    ...
    docker exec -it ${PROXY2} bash -c "/tmp/.acnodal/bin/pfc_add.sh ${PROXY_NIC2} ${GROUP_ID} ${SERVICE_ID2} ${PASSWD2} 0 0 ${SERVICE_PROTO2} ${PROXY_IP2} ${PROXY_PORT2} ${SERVICE_IP2} ${SERVICE_PORT2}" > /dev/null
    PROXY_TUN_PORT2=$(docker exec -it ${PROXY2} bash -c "/tmp/.acnodal/bin/cli_tunnel get ${TUNNEL_ID2}" | grep ${TUNNEL_ID2} | awk '{print $3}' | sed 's/:/ /g' | awk '{print $2}')
    docker exec -it ${NODE2} bash -c "/tmp/.acnodal/bin/pfc_add.sh -p ${NODE_NIC2} ${GROUP_ID} ${SERVICE_ID2} ${PASSWD2} ${PROXY_TUN_IP2} ${PROXY_TUN_PORT2} ${SERVICE_PROTO2} ${PROXY_IP2} ${PROXY_PORT2} ${SERVICE_IP2} ${SERVICE_PORT2}" > /dev/null
fi
...
for (( i=1; i<10; i++ ))
do
    if [ "$(docker exec -it ${PROXY2} bash -c "/tmp/.acnodal/bin/cli_tunnel get ${TUNNEL_ID2}" | grep "TUN" | grep ${TUNNEL_ID2} | grep -v "0.0.0.0:0")" ] ; then
        break
    fi
    echo "."
    sleep 1
...
if [ ! "$(docker exec -it ${PROXY} bash -c "/tmp/.acnodal/bin/cli_tunnel get ${TUNNEL_ID}" | grep "TUN" | grep ${TUNNEL_ID} | grep -v "0.0.0.0:0")" ] ; then
    ...
    RETURN=1
elif [ ! "$(docker exec -it ${PROXY2} bash -c "/tmp/.acnodal/bin/cli_tunnel get ${TUNNEL_ID2}" | grep "TUN" | grep ${TUNNEL_ID2} | grep -v "0.0.0.0:0")" ] ; then
...
#    tail -n60 /sys/kernel/debug/tracing/trace
    ...
    TMP=$(./${SERVICE_TYPE2}_check.sh ${CLIENT} ${PROXY_IP2} ${PROXY_PORT2} ${SERVICE_ID2})                                                                                                                                                                                       
    if [ "${VERBOSE}" ]; then                                                                                                                                                                                                                                                 
        echo "${TMP}"                                                                                                                                                                                                                                                         
    fi                                                                                                                                                                                                                                                                        
...
if [ "${VERBOSE}" ]; then
    ...
    docker exec -it ${PROXY2} bash -c "/tmp/.acnodal/bin/pfc_delete.sh ${GROUP_ID} ${SERVICE_ID2}"
    docker exec -it ${NODE2} bash -c "/tmp/.acnodal/bin/pfc_delete.sh ${GROUP_ID} ${SERVICE_ID2}"
    ...
    docker exec -it ${PROXY2} bash -c "/tmp/.acnodal/bin/pfc_stop.sh ${PROXY_NIC2}"
    docker exec -it ${NODE} bash -c "/tmp/.acnodal/bin/pfc_stop.sh ${NODE_NIC}"
    docker exec -it ${NODE2} bash -c "/tmp/.acnodal/bin/pfc_stop.sh ${NODE_NIC2}"
    ...
    docker exec -it ${PROXY2} bash -c "/tmp/.acnodal/bin/pfc_delete.sh ${GROUP_ID} ${SERVICE_ID2}" > /dev/null
    docker exec -it ${NODE2} bash -c "/tmp/.acnodal/bin/pfc_delete.sh ${GROUP_ID} ${SERVICE_ID2}" > /dev/null

    # Stop PFC
    docker exec -it ${PROXY} bash -c "/tmp/.acnodal/bin/pfc_stop.sh ${PROXY_NIC}" > /dev/null
    docker exec -it ${PROXY2} bash -c "/tmp/.acnodal/bin/pfc_stop.sh ${PROXY_NIC2}" > /dev/null
    docker exec -it ${NODE} bash -c "/tmp/.acnodal/bin/pfc_stop.sh ${NODE_NIC}" > /dev/null
    docker exec -it ${NODE2} bash -c "/tmp/.acnodal/bin/pfc_stop.sh ${NODE_NIC2}" > /dev/null
