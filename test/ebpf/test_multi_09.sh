...
docker exec -itd ${NODE} bash -c "python3 gue_ping_svc_once.py ${NIC} ${PROXY_TUN_IP} ${PROXY_TUN_PORT} ${NODE_TUN_PORT} ${GROUP_ID} ${SERVICE_ID} ${PASSWD}"

######## CONFIGURE NODE2 ########
### Install & configure PFC (<node> <iface> <role> <mode> ...)
NIC="eth1"

docker exec -it ${NODE2} bash -c "attach_tc.sh ${NIC}"
# cli_cfg set <idx> <id> <flags> <name>
docker exec -it ${NODE2} bash -c "cli_cfg set ${NIC} 0 2 9 '${NODE2} RX' && cli_cfg set ${NIC} 1 2 8 '${NODE2} TX'"
# check
if [ "${VERBOSE}" ]; then
    echo ""
    docker exec -it ${NODE2} bash -c "cli_cfg get all"
fi

docker exec -itd ${NODE2} bash -c "python3 gue_ping_svc_auto.py ${DELAY} &> /tmp/gue_ping.log"
...
if [ "${VERBOSE}" ]; then
    echo ""
    docker exec -it ${NODE2} bash -c "cli_tunnel get all"
fi
...
if [ "${VERBOSE}" ]; then
    echo ""
    docker exec -it ${NODE2} bash -c "cli_service get all"
fi

docker exec -itd ${NODE2} bash -c "python3 gue_ping_svc_once.py ${NIC} ${PROXY_TUN_IP} ${PROXY_TUN_PORT2} ${NODE_TUN_PORT2} ${GROUP_ID} ${SERVICE_ID2} ${PASSWD2}"
