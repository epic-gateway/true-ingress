...
#    tail -n60 /sys/kernel/debug/tracing/trace
    ...
    docker exec -it ${CLIENT} bash -c "nping -c1 --df --data-length 1460 --tcp --source-port 10000 --dest-port ${PROXY_PORT} ${PROXY_IP}"
    docker exec -it ${CLIENT} bash -c "nping -c1 --df --data-length 1400 --tcp --source-port 10000 --dest-port ${PROXY_PORT} ${PROXY_IP}"
