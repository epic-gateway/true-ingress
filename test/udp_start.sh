...
#docker exec -it ${NODE} bash -c "echo 5 > /proc/sys/net/ipv4/ip_no_pmtu_disc"
sudo bash -c "echo 0 > /proc/sys/net/ipv4/ip_no_pmtu_disc"
docker exec -itd ${NODE} bash -c "python3 /tmp/.acnodal/bin/udp_server.py ${IP} ${PORT} ${HOME} &> ${HOME}/log"
#docker exec -it ${NODE} bash -c "python3 /tmp/.acnodal/bin/udp_server.py ${IP} ${PORT} ${HOME}"
