...
if [ "${VERBOSE}" ]; then
    echo -e "\nFORWARDING.LNX.ADD : SERVICE_ID='${SERVICE_ID}' NODE='${NODE}' PROXY='${PROXY}' PROTO='${PROTO}' SERVICE_IP='${SERVICE_IP}' SERVICE_PORT='${SERVICE_PORT}' PROXY_IP='${PROXY_IP}' PROXY_PORT='${PROXY_PORT}' CLIENT='${CLIENT}'"
fi
...
echo -e "\n==============================================="
echo "# FORWARDING.LNX.ADD [2/${STEPS}] : Reachability check"
...
echo -e "\n==============================================="
echo "# FORWARDING.LNX.ADD [3/${STEPS}] : (OUT OF ORDER) Start tunnel ping (in background) for service ID ${SERVICE_ID} every ${DELAY} seconds"
...
echo -e "\n==============================================="
echo "# FORWARDING.LNX.ADD [5/${STEPS}] : Configure EGW on '${PROXY}'"
# syntax:  $0  <node>   <service-id>  <proto>  <service-ip>  <service-port>  <real-ip>  <real-port>  <proxy-ip>  <proxy-port>  <foo-ip>
./egw_lnx_setup.sh ${PROXY} ${SERVICE_ID} ${PROTO} ${SERVICE_IP} ${SERVICE_PORT} ${REAL_IP} ${REAL_PORT} ${PROXY_IP} ${PROXY_PORT} ${FOO_IP}
# here should we receive EGW response
#PROXY_IP=$7
#PROXY_PORT=$8

echo -e "\n==============================================="
echo "# FORWARDING.LNX.ADD [6/${STEPS}] : Configure PFC on '${NODE}'"
# syntax:  $0  <node>  <service-id>  <proto>  <service-ip>  <service-port>  <tunnel-ip>   <tunnel-port>  <foo-ip>
./pfc_lnx_setup.sh ${NODE} ${SERVICE_ID} ${PROTO} ${SERVICE_IP} ${SERVICE_PORT} ${PROXY_IFIP} ${TUNNEL_PORT} ${FOO_IP}

echo -e "\n==============================================="
echo "# FORWARDING.LNX.ADD [7/${STEPS}] : CHECK:"
